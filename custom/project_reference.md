...
- Undead Paladin: **Restricted to real human players only (account ID 1 / Malikyth)**; strictly excluded from all AI playerbot character creation via `RandomPlayerbotFactory::IsValidRaceClassCombination`
...
| Date | Summary |
|------|---------|
| 2026-08-17 | Restored and compiled `item_heirloom_portalstone` (Portalstone of the Wayfarer, item 90124) in `mod_assistant`. Provides instant Bound Inn (Hearthstone) teleport and interactive portals to all major capitals (Stormwind, Ironforge, Darnassus, Exodar, Orgrimmar, Undercity, Thunder Bluff, Silvermoon, Shattrath, Dalaran, Booty Bay, Ratchet, Gadgetzan). Set Expansion = 2 with MaxPlayerLevel = 60 so all portals and world maps are fully accessible. |
